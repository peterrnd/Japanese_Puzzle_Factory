...
function swapPuzzle(e){
   if(confirm("You will lose all of your work on the puzzle! Continue?")){
      ...
      // Switch statement to determine which puzzle button triggered this function
      switch(puzzleID) {
         case "puzzle1":
            document.getElementById("puzzle").innerHTML = drawPuzzle(puzzle1Hint, puzzle1Rating, puzzle1);
            break;
   ...
   function setupPuzzle(){
      ...
      for (var i = 0; i < puzzleCells.length; i++){
         puzzleCells[i].style.backgroundColor =
         "rgb(233, 207, 29)";
         //set the cell background color in response to mousedown event
      ...
      function() {
        ...
        setTimeout(
            function() {
               ...
               for(var i = 0; i < puzzleCells.length; i++) {
                  console.log(puzzleCells[0].style.backgroundColor);
...
function setBackground(e){
   ...
   // Set the cell background based on the keyboard key
   if(e.shiftKey) {
   ...
   }// end of for loop creating event listener for every puzzle cell
